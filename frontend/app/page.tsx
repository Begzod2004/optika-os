...
export default function OptikaOS() {
  ...
  async function cancelOrder(id: number) {
    const order = orders.find(item => item.id === id);
    const advanceNote = order && order.paid > 0 ? ` To'langan avans (${format(order.paid)}) kassada qoladi.` : "";
    if (!window.confirm(`Buyurtmani bekor qilishni tasdiqlaysizmi? Mahsulot omborga qaytadi.${advanceNote}`)) return;
    if (busy) return; setBusy(true);
...
function Cash({revenue,debt,shift,expenses,branchName,open,close,expense}:{revenue:number;debt:number;shift:CashShift | null;expenses:ExpenseRec[];branchName?:string;open:()=>void;close:()=>void;expense:()=>void}) {
  const MOVE_LABELS: Record<string, string> = { SALE_PAYMENT: "Sotuv", ORDER_ADVANCE: "Buyurtma avansi", REPAIR_PAYMENT: "Servis to'lovi", CUSTOMER_DEBT_PAYMENT: "Qarz to'lovi", REFUND: "Qaytarish", SUPPLIER_PAYMENT: "Supplierga to'lov", EXPENSE: "Xarajat" };
