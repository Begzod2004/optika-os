...
type Order = { id: number; customer: string; item: string; total: number; paid: number; status: "Yangi" | "Ishda" | "Tayyor" | "Topshirildi" | "Bekor qilindi"; date: string };
...
type SaleRecord = { id: number; customer: Customer | null; total: number; paid: number; items: { product: Product; quantity: number; unit_price: number }[] };
...
const seedProducts: Product[] = [
  { id: 1, name: "RB 3447 Round", brand: "Ray-Ban", type: "Rama", price: 850000, stock: 6, min: 3 },
  { id: 2, name: "Blue Cut 1.60", brand: "Essilor", type: "Linza", price: 300000, stock: 18, min: 8 },
  { id: 3, name: "Classic Black Case", brand: "Optika", type: "Aksessuar", price: 50000, stock: 4, min: 5 },
  { id: 4, name: "Air Optix Aqua", brand: "Alcon", type: "Kontakt linza", price: 180000, stock: 12, min: 6 },
  { id: 5, name: "Oakley Holbrook", brand: "Oakley", type: "Rama", price: 1250000, stock: 2, min: 3 },
];
const seedCustomers: Customer[] = [
  { id: 1, name: "Aziz Akbarov", phone: "+998 90 123 45 67", debt: 500000, visits: 8 },
  { id: 2, name: "Madina Karimova", phone: "+998 93 555 20 20", debt: 0, visits: 3 },
  { id: 3, name: "Jasur Aliyev", phone: "+998 99 741 11 22", debt: 390000, visits: 2 },
];
const seedOrders: Order[] = [
  { id: 1582, customer: "Aziz Akbarov", item: "RB 3447 Round + Blue Cut", total: 1500000, paid: 500000, status: "Tayyor", date: "Bugun, 09:29" },
  { id: 1581, customer: "Madina Karimova", item: "Oakley Holbrook", total: 2250000, paid: 2250000, status: "Ishda", date: "Bugun, 09:08" },
...
export default function OptikaOS() {
  ...
  const [products, setProducts] = useSavedState("optika-products", seedProducts);
  const [customers, setCustomers] = useSavedState("optika-customers", seedCustomers);
  const [orders, setOrders] = useSavedState("optika-orders", seedOrders);
  ...
  const loadFromApi = useCallback(async () => {
    try {
      ...
      setCustomers(data.customers.map((customer: {id:number;name:string;phone:string;debt:number}) => ({ ...customer, visits: 0 })));
      setProducts(data.products.map((product: {id:number;name:string;brand:string;category:string;sale_price:number;stock:number;minimum_stock:number}) => ({ id: product.id, name: product.name, brand: product.brand, type: product.category, price: product.sale_price, stock: product.stock, min: product.minimum_stock })));
      setSuppliers(data.suppliers || []);
      setCashShift(data.cash_shift || null);
      const salesResponse = await fetch(`${apiUrl}/sales`, { headers: { Authorization: `Bearer ${token}` } });
      if (salesResponse.ok) {
        const salesData = (await salesResponse.json()).data;
        setSales(salesData.map((sale: {id:number;customer:{id:number;name:string;phone:string;debt:number}|null;total:number;paid:number;items:{product:{id:number;name:string;brand:string;category:string;sale_price:number;stock:number;minimum_stock:number};quantity:number;unit_price:number}[]}) => ({ id: sale.id, customer: sale.customer ? { ...sale.customer, visits: 0 } : null, total: sale.total, paid: sale.paid, items: sale.items.map(item => ({ product: { id: item.product.id, name: item.product.name, brand: item.product.brand, type: item.product.category, price: item.product.sale_price, stock: item.product.stock, min: item.product.minimum_stock }, quantity: item.quantity, unit_price: item.unit_price })) })));
      }
      const financeResponse = await fetch(`${apiUrl}/reports/finance`, { headers: { Authorization: `Bearer ${token}` } });
      if (financeResponse.ok) setFinance((await financeResponse.json()).data);
      const repairsResponse = await fetch(`${apiUrl}/repairs`, { headers: { Authorization: `Bearer ${token}` } });
      if (repairsResponse.ok) {
        const repairData = (await repairsResponse.json()).data;
        setRepairs(repairData.map((repair: {id:number;customer:{id:number;name:string;phone:string;debt:number};device:string;issue:string;estimated_cost:number;paid:number;balance:number;status:string;technician_note:string|null;due_date:string|null}) => ({ ...repair, customer: { ...repair.customer, visits: 0 } })));
      }
      const usersResponse = await fetch(`${apiUrl}/users`, { headers: { Authorization: `Bearer ${token}` } });
      if (usersResponse.ok) setTeam((await usersResponse.json()).data);
      const branchesResponse = await fetch(`${apiUrl}/branches`, { headers: { Authorization: `Bearer ${token}` } });
      if (branchesResponse.ok) setBranches((await branchesResponse.json()).data);
      const opticalCasesResponse = await fetch(`${apiUrl}/optical-cases`, { headers: { Authorization: `Bearer ${token}` } });
      if (opticalCasesResponse.ok) setOpticalCases((await opticalCasesResponse.json()).data);
      const notificationsResponse = await fetch(`${apiUrl}/notifications`, { headers: { Authorization: `Bearer ${token}` } });
      if (notificationsResponse.ok) setSystemNotifications((await notificationsResponse.json()).data);
      ...
      setOrders(data.orders.map((order: {id:number;customer:{name:string};product:{name:string};total:number;paid:number;status:string;created_at:string}) => ({ id: order.id, customer: order.customer.name, item: order.product.name, total: order.total, paid: order.paid, status: mapStatus[order.status] || "Yangi", date: new Date(order.created_at).toLocaleDateString("uz-UZ") })));
      setBackendOnline(true);
    } catch { setBackendOnline(false); }
  }, [setCustomers, setOrders, setProducts, token]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      const response = await fetch(`${apiUrl}/search?q=${encodeURIComponent(query)}`, { headers: { Authorization: `Bearer ${token}` } });
      if (response.ok) setSearchResults((await response.json()).data);
    }, 220);
    return () => window.clearTimeout(timer);
  }, [query, token]);

  const revenue = useMemo(() => orders.reduce((sum, order) => sum + order.paid, 0), [orders]);
  ...
  async function checkout() {
    if (!cart.length) return setNotice("Avval savatchaga mahsulot qosh­ing");
    if (!backendOnline) return setNotice("Backend bilan aloqa yoq. API serverni ishga tushiring.");
    const response = await fetch(`${apiUrl}/sales`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ paid: cartTotal, payment_method: "CASH", items: cart.map(line => ({ product_id: line.id, quantity: line.qty })) }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Sotuv saqlanmadi");
    setCart([]); await loadFromApi(); setNotice(`${format(cartTotal)} sotuv bazaga saqlandi`);
  }
  async function addCustomer(event: FormEvent<HTMLFormElement>) {
    ...
    if (!name || !phone || !backendOnline) return setNotice("Mijozni saqlash uchun API ulanishi kerak");
    const response = await fetch(`${apiUrl}/customers`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name, phone }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Mijoz saqlanmadi");
    setModal(null); await loadFromApi(); setNotice(`${name} mijoz sifatida bazaga qoshildi`);
  }
  async function addOrder(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const data = new FormData(event.currentTarget); const customer = String(data.get("customer")); const product = products.find(item => item.id === Number(data.get("product")));
    const selected = customers.find(item => item.name === customer);
    if (!selected || !product || !backendOnline) return setNotice("Buyurtma uchun API ulanishi kerak");
    const response = await fetch(`${apiUrl}/orders`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ customer_id: selected.id, product_id: product.id, paid: Number(data.get("paid") || 0) }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Buyurtma saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Buyurtma bazaga yaratildi va mahsulot rezerv qilindi");
  }
  async function addRepair(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!backendOnline) return setNotice("Servis buyurtmasi uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/repairs`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ customer_id: Number(data.get("customer")), device: data.get("device"), issue: data.get("issue"), estimated_cost: Number(data.get("estimated_cost")), paid: Number(data.get("paid") || 0), due_date: data.get("due_date") ? new Date(String(data.get("due_date"))).toISOString() : null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Servis buyurtmasi saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Servis buyurtmasi qabul qilindi");
  }
  async function advanceRepair(repair: Repair) {
    ...
    const response = await fetch(`${apiUrl}/repairs/${repair.id}/status`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ status: next }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Servis holati yangilanmadi");
    await loadFromApi(); setNotice("Servis buyurtmasi holati yangilandi");
  }
  async function createTelegramLink(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${apiUrl}/customers/${customerId}/telegram-link`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
    if (!response.ok) return setNotice((await response.json()).detail || "Telegram havolasi yaratilmagan");
    const result = (await response.json()).data;
    const qr = await QRCode.toDataURL(result.url, { width: 280, margin: 2, color: { dark: "#087653", light: "#ffffff" } });
    setTelegramLink({ customer: result.customer.name, url: result.url, qr });
    try { await navigator.clipboard.writeText(result.url); setNotice("Telegram havolasi nusxalandi — mijozga yuboring"); } catch { setNotice("Havola yaratildi, uni nusxalang"); }
  }
  async function getOpticalAdvice(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const customerId = Number(data.get("customer"));
    const response = await fetch(`${apiUrl}/customers/${customerId}/optical-advice`, { headers: { Authorization: `Bearer ${token}` } });
    if (!response.ok) return setNotice((await response.json()).detail || "Tavsiyani yuklab bo'lmadi");
    const result = (await response.json()).data;
    setOpticalAdvice({ ...result, customer: { ...result.customer, visits: 0 } });
  }
  ...
  async function createOpticalCase(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${apiUrl}/optical-cases`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ customer_id: Number(data.get("customer")), frame_product_id: frame ? Number(frame) : null, chief_complaint: data.get("chief_complaint") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Optik karta yaratilmadi");
    setModal(null); await loadFromApi(); setNotice("Ramka tanlandi. Endi ko'z tekshiruvi bosqichiga o'ting.");
  }
  async function saveOpticalStep(event: FormEvent<HTMLFormElement>, endpoint: string, success: string) {
    event.preventDefault();
    if (!selectedOpticalCase) return;
    const data = new FormData(event.currentTarget);
    ...
    const response = await fetch(`${apiUrl}/optical-cases/${selectedOpticalCase.id}/${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
    if (!response.ok) return setNotice((await response.json()).detail || "Bosqich saqlanmadi");
    setModal(null); await loadFromApi(); setNotice(success);
  }
  async function advanceLab(caseItem: OpticalCase) {
    const next = caseItem.lab?.status === "SENT" ? "RECEIVED" : caseItem.lab?.status === "RECEIVED" ? "QC_PASSED" : caseItem.lab?.status === "QC_PASSED" ? "READY" : caseItem.lab?.status === "READY" ? "DELIVERED" : null;
    if (!next) return;
    const response = await fetch(`${apiUrl}/optical-cases/${caseItem.id}/lab/status`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ status: next }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Lab holati yangilanmadi");
    await loadFromApi(); setNotice("Laboratoriya/QC holati yangilandi");
  }
  async function addBranch(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/branches`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name: data.get("name"), code: data.get("code"), address: data.get("address") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Filial saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Yangi filial yaratildi");
  }
  async function addTeamUser(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/users`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ username: data.get("username"), password: data.get("password"), role: data.get("role") }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Xodim saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Yangi xodim va roli yaratildi");
  }
  async function addStock(event: FormEvent<HTMLFormElement>) {
    ...
    if (!amount || !backendOnline) return setNotice("Kirim uchun API ulanishi kerak");
    const response = await fetch(`${apiUrl}/inventory/${productId}/receive`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ quantity: amount }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Kirim saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Kirim bazaga saqlandi va ombor yangilandi");
  }
  async function advanceOrder(id: number) {
    if (!backendOnline) return setNotice("Backend bilan aloqa yoq");
    const response = await fetch(`${apiUrl}/orders/${id}/advance`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
    if (!response.ok) return setNotice((await response.json()).detail || "Status yangilanmadi");
    await loadFromApi(); setNotice("Buyurtma holati yangilandi");
  }
  async function cancelOrder(id: number) {
    if (!window.confirm("Buyurtmani bekor qilishni tasdiqlaysizmi? Mahsulot omborga qaytadi.")) return;
    const response = await fetch(`${apiUrl}/orders/${id}/cancel`, { method: "POST", headers: { Authorization: `Bearer ${token}` } });
    if (!response.ok) return setNotice((await response.json()).detail || "Buyurtma bekor qilinmadi");
    await loadFromApi(); setNotice("Buyurtma bekor qilindi va rezerv omborga qaytdi");
  }
  async function addPrescription(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedCustomer || !backendOnline) return setNotice("Retsept uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const payload = Object.fromEntries([...data.entries()].filter(([, value]) => value !== ""));
    const response = await fetch(`${apiUrl}/customers/${selectedCustomer.id}/prescriptions`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(payload) });
    if (!response.ok) return setNotice((await response.json()).detail || "Retsept saqlanmadi");
    setModal(null); setNotice(`${selectedCustomer.name} uchun retsept saqlandi`);
  }
  async function addSupplier(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!backendOnline) return setNotice("Supplier uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/suppliers`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name: data.get("name"), phone: data.get("phone") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Supplier saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Supplier bazaga qoshildi");
  }
  async function receiveCustomerPayment(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedCustomer || !backendOnline) return setNotice("To'lov uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/customers/${selectedCustomer.id}/payments`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ amount: Number(data.get("amount")), comment: data.get("comment") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "To'lov saqlanmadi");
    setModal(null); await loadFromApi(); setNotice(`${selectedCustomer.name}dan qarz to'lovi qabul qilindi`);
  }
  async function paySupplier(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedSupplier || !backendOnline) return setNotice("To'lov uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/suppliers/${selectedSupplier.id}/payments`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ amount: Number(data.get("amount")), comment: data.get("comment") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Supplier to'lovi saqlanmadi");
    setModal(null); await loadFromApi(); setNotice(`${selectedSupplier.name}ga to'lov saqlandi`);
  }
  async function addPurchase(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!backendOnline) return setNotice("Kirim uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const quantity = Number(data.get("quantity")); const unitCost = Number(data.get("unit_cost")); const paid = Number(data.get("paid") || 0);
    const response = await fetch(`${apiUrl}/purchases`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ supplier_id: Number(data.get("supplier")), paid, invoice_no: data.get("invoice_no") || null, items: [{ product_id: Number(data.get("product")), quantity, unit_cost: unitCost }] }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Supplier kirimi saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Supplierdan kirim, qoldiq va qarzdorlik yangilandi");
  }
  async function changeCashShift(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!backendOnline) return setNotice("Kassa uchun API ulanishi kerak");
    const amount = Number(new FormData(event.currentTarget).get("amount") || 0);
    const isOpening = modal === "cash-open";
    const endpoint = isOpening ? `${apiUrl}/cash-shifts/open` : `${apiUrl}/cash-shifts/${cashShift?.id}/close`;
    const body = isOpening ? { opening_amount: amount } : { actual_amount: amount };
    ...
    if (!response.ok) return setNotice((await response.json()).detail || "Kassa amali saqlanmadi");
    setModal(null); await loadFromApi(); setNotice(isOpening ? "Kassa smenasi ochildi" : "Kassa smenasi yopildi");
  }
  async function returnSale(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedReturn || !backendOnline) return setNotice("Qaytarish uchun API ulanishi kerak");
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/sales/${selectedReturn.saleId}/return`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ product_id: selectedReturn.product.id, quantity: Number(data.get("quantity")), disposition: data.get("disposition"), reason: data.get("reason") }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Qaytarish saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Qaytarish bazaga saqlandi, ombor va kassa yangilandi");
  }
  async function addExpense(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/expenses`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ category: data.get("category"), amount: Number(data.get("amount")), description: data.get("description") || null }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Xarajat saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Xarajat kassa va audit logga saqlandi");
  }
  async function addProduct(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/products`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ name: data.get("name"), brand: data.get("brand") || "Optika", category: data.get("category"), sale_price: Number(data.get("sale_price")), stock: Number(data.get("stock")), minimum_stock: Number(data.get("minimum_stock")) }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Mahsulot saqlanmadi");
    setModal(null); await loadFromApi(); setNotice("Yangi mahsulot omborga qoshildi");
  }

  async function signIn(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const response = await fetch(`${apiUrl}/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: data.get("username"), password: data.get("password") }) });
    if (!response.ok) return setNotice((await response.json()).detail || "Kirish amalga oshmadi");
    const result = (await response.json()).data;
    localStorage.setItem("optika-token", result.access_token);
    localStorage.setItem("optika-user", JSON.stringify(result.user));
    setUser(result.user); setToken(result.access_token);
  }

  function signOut() {
    localStorage.removeItem("optika-token"); localStorage.removeItem("optika-user");
  ...
  return <main className="app-shell">
    <aside className={`side ${menu ? "show" : ""}`}>
      ...
      <div className="side-bottom"><small>CHILONZOR FILIALI</small><b>Aziz Admin</b><span>Bugun ochiq smena</span></div>
    </aside>
    <section className="main">
      <header className="topbar"><button className="menu" onClick={() => setMenu(!menu)}><Menu/></button><div className="search-wrap"><div className="global-search"><Search size={18}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Mijoz, buyurtma yoki mahsulot qidiring"/>{query && <span>{searchResults.length} topildi</span>}</div>{searchResults.length > 0 && <div className="search-popover">{searchResults.map(result => <button key={`${result.type}-${result.id}`} onClick={() => { setQuery(""); setNotice(`${result.title} tanlandi`); }}><b>{result.title}</b><span>{result.type} · {result.subtitle}</span></button>)}</div>}</div><span className={`api-status ${backendOnline ? "online" : ""}`}>{backendOnline ? "API ulangan" : "Ulanmoqda"}</span><button className="bell" onClick={() => setNotice("3 ta tayyor buyurtma sizni kutmoqda")}><Bell size={19}/><i/></button><button className="account" onClick={signOut} title="Chiqish"><span className="avatar">{user.username.slice(0,2).toUpperCase()}</span><small>{user.role}</small></button></header>
      <div className="page">
        {notice && <button className="notice" onClick={() => setNotice("")}><b>✓</b>{notice}<X size={15}/></button>}
        {section === "Bosh sahifa" && <Dashboard revenue={revenue} debt={debt} customers={customers.length} orders={orders} lowStock={lowStock} go={setSection} setModal={setModal} statusTone={statusTone}/>} 
        ...
        {systemNotifications.length > 0 && <section className="system-alerts">{systemNotifications.slice(0, 3).map(item => <button key={`${item.type}-${item.entity_id}`} className={item.severity === "warning" ? "warning" : "success"} onClick={() => setSection(item.type === "LOW_STOCK" ? "Ombor" : "Buyurtmalar")}><Bell size={15}/><span><b>{item.title}</b>{item.message}</span><ChevronRight size={15}/></button>)}</section>}
        {section === "Sotuv" && <POS products={products} cart={cart} total={cartTotal} query={query} add={addToCart} qty={updateQty} checkout={checkout} setModal={setModal}/>} 
        ...
        {section === "Kassa" && <Cash revenue={revenue} debt={debt} orders={orders} shift={cashShift} open={() => setModal("cash-open")} close={() => setModal("cash-close")} expense={() => setModal("expense")}/>} 
    ...
    {modal && <Modal title={modal === "customer" ? "Yangi mijoz" : modal === "order" ? "Yangi buyurtma" : modal === "stock" ? "Omborga kirim" : modal === "product" ? "Yangi mahsulot" : modal === "supplier" ? "Yangi supplier" : modal === "cash-open" ? "Kassa smenasini ochish" : modal === "cash-close" ? "Kassa smenasini yopish" : modal === "return" ? `${selectedReturn?.product.name || "Mahsulot"} qaytarish` : modal === "expense" ? "Xarajat kiritish" : modal === "customer-payment" ? `${selectedCustomer?.name || "Mijoz"} qarzi` : modal === "supplier-payment" ? `${selectedSupplier?.name || "Supplier"} to'lovi` : modal === "purchase" ? "Supplierdan kirim" : `${selectedCustomer?.name || "Mijoz"} retsepti`} close={() => setModal(null)}>
      ...
      {modal === "expense" && <form className="form" onSubmit={addExpense}><label>Kategoriya<select name="category"><option>Ish haqi</option><option>Transport</option><option>Kommunal</option><option>Marketing</option><option>Tamir</option><option>Boshqa</option></select></label><label>Summa<input name="amount" required type="number" min="1"/></label><label>Izoh<input name="description" placeholder="Xarajat tavsifi"/></label><button className="primary">Xarajatni saqlash</button></form>}
...
function LoginScreen({onSubmit,notice,clearNotice}:{onSubmit:(event:FormEvent<HTMLFormElement>)=>void;notice:string;clearNotice:()=>void}) { return <main className="login-screen"><section className="login-card"><div className="login-logo"><span>O</span><div>OPTIKA<small>BUSINESS OS</small></div></div><div><p>XUSH KELIBSIZ</p><h1>Tizimga kiring</h1><span>Optika ish jarayonini bir joydan boshqaring.</span></div>{notice && <button className="login-error" onClick={clearNotice}>{notice}<X size={15}/></button>}<form className="form" onSubmit={onSubmit}><label>Login<input name="username" required autoFocus placeholder="admin"/></label><label>Parol<input name="password" required type="password" placeholder="••••••••"/></label><button className="primary">Kirish</button></form><small className="demo-hint">Demo: admin / admin123</small></section></main> }
