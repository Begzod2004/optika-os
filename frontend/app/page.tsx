...
export default function OptikaOS() {
  ...
  const loadFromApi = useCallback(async () => {
    try {
      ...
      setOrders(data.orders.map((order: {id:number;customer:{name:string};product:{name:string}|null;item_name:string|null;total:number;paid:number;status:string;created_at:string}) => ({ id: order.id, customer: order.customer.name, item: order.item_name || order.product?.name || "Mahsulot", total: order.total, paid: order.paid, status: mapStatus[order.status] || "Yangi", date: new Date(order.created_at).toLocaleDateString("uz-UZ") })));
  ...
  async function addOrder(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const data = new FormData(event.currentTarget); const customer = String(data.get("customer"));
    const selected = customers.find(item => item.name === customer);
    if (!selected || !backendOnline) return setNotice("Buyurtma uchun API ulanishi kerak");
    const paid = Number(data.get("paid") || 0);
    let body: Record<string, unknown>;
    if (manualOrder) {
      const itemName = String(data.get("item_name") || "").trim();
      if (!itemName) return setNotice("Mahsulot nomini kiriting");
      ...
      if (!product) return setNotice("Ro'yxatdan mahsulot tanlang yoki qo'lda kiriting");
      body = { customer_id: selected.id, product_id: product.id, paid };
    }
    const response = await fetch(`${apiUrl}/orders`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(body) });
    if (!response.ok) return setNotice((await response.json()).detail || "Buyurtma saqlanmadi");
    setModal(null); setManualOrder(false); await loadFromApi(); setNotice(manualOrder ? "Qo'lda buyurtma bazaga yaratildi" : "Buyurtma bazaga yaratildi va mahsulot rezerv qilindi");
  }
