...
export default function OptikaOS() {
  ...
  return <main className="app-shell">
    <aside className={`side ${menu ? "show" : ""}`}>
      ...
      <div className="side-bottom"><small>OPTIKA OS</small><b>{user.username.charAt(0).toUpperCase() + user.username.slice(1)}</b><span>{user.branch ? user.branch.name : user.role === "SELLER" ? "Filial biriktirilmagan" : "Barcha filiallar"}</span><span>{cashShift ? "Smena ochiq" : "Smena yopiq"}</span></div>
    </aside>
    <section className="main">
      ...
      <div className="page">
        ...
        {section === "Bosh sahifa" && <Dashboard revenue={todayRevenue} debt={debt} customers={customers.length} debtors={customers.filter(c => c.debt > 0).length} orders={orders} lowStock={lowStock} go={goSection} setModal={setModal} statusTone={statusTone} userName={user.username}/>}
