...
type SessionUser = { id?: number; username: string; role: string; full_name?: string | null; phone?: string | null; telegram_username?: string | null; photo?: string | null; branch?: { id: number; name: string } | null };
type Supplier = { id: number; name: string; phone: string | null; balance: number };
type CashMove = { id: number; movement_type: string; amount: number; note: string | null; created_at: string };
type CashShift = { id: number; opening_amount: number; expected_amount: number | null; actual_amount: number | null; difference: number | null; status: string; opened_at: string; expected?: number; movements?: CashMove[] };
type SaleRecord = { id: number; customer: Customer | null; total: number; paid: number; created_at?: string; items: { product: Product; quantity: number; unit_price: number; returned: number }[] };
...
type TeamUser = { id:number; username:string; role:string; active:boolean; full_name?:string | null; phone?:string | null; telegram_username?:string | null; photo?:string | null };
...
type ExpenseRec = { id: number; category: string; amount: number; description: string | null; cash_shift_id: number | null; created_at: string };
type PurchaseRec = { id: number; supplier: Supplier; total: number; paid: number; balance: number; invoice_no: string | null; created_at: string };
type PrescriptionRec = { id: number; od_sph: string | null; od_cyl: string | null; od_axis: number | null; os_sph: string | null; os_cyl: string | null; os_axis: number | null; pd: number | null; note: string | null; reminder_at: string | null; created_at: string };
...
export default function OptikaOS() {
  ...
  const [expensesList, setExpensesList] = useState<ExpenseRec[]>([]);
  const [purchases, setPurchases] = useState<PurchaseRec[]>([]);
  ...
  const [modal, setModal] = useState<"customer" | "order" | "stock" | "product" | "prescription" | "supplier" | "cash-open" | "cash-close" | "return" | "expense" | "customer-payment" | "supplier-payment" | "purchase" | "repair" | "user" | "branch" | "optical-case" | "eye-exam" | "centration" | "lens-config" | "lab-job" | "lab-qc" | "profile" | null>(null);
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(null);
  const [selectedSupplier, setSelectedSupplier] = useState<Supplier | null>(null);
  const [cashShift, setCashShift] = useState<CashShift | null>(null);
  const [selectedReturn, setSelectedReturn] = useState<{ saleId: number; product: Product; quantity: number; returned: number } | null>(null);
  const [selectedOpticalCase, setSelectedOpticalCase] = useState<OpticalCase | null>(null);
  const [prescriptions, setPrescriptions] = useState<PrescriptionRec[]>([]);
  ...
  useEffect(() => {
    if (modal !== "prescription" || !selectedCustomer) { setPrescriptions([]); return; }
    void (async () => { const res = await api<PrescriptionRec[]>(`/customers/${selectedCustomer.id}/prescriptions`); if (res.ok) setPrescriptions(res.data); })();
  }, [modal, selectedCustomer, api]);
  ...
  const loadFromApi = useCallback(async () => {
    ...
    const [salesRes, financeRes, repairsRes, usersRes, branchesRes, casesRes, notifsRes, expensesRes, purchasesRes] = await Promise.all([
      api<{id:number;customer:{id:number;name:string;phone:string;debt:number}|null;total:number;paid:number;created_at:string;items:{product:{id:number;name:string;brand:string;category:string;sale_price:number;stock:number;minimum_stock:number};quantity:number;unit_price:number;returned:number}[]}[]>("/sales"),
    ...
    if (expensesRes.ok) setExpensesList(expensesRes.data);
    if (purchasesRes.ok) setPurchases(purchasesRes.data);
  ...
  const todayRevenue = useMemo(() => sales.filter(sale => isToday(sale.created_at)).reduce((sum, sale) => sum + sale.paid, 0) + orders.filter(order => isToday(order.createdAt) && order.status !== "Bekor qilindi").reduce((sum, order) => sum + order.paid, 0), [sales, orders]);
  ...
  async function checkout() {
    ...
    const res = await api("/sales", { method: "POST", body: { paid: cartTotal, payment_method: payMethod, items: cart.map(line => ({ product_id: line.id, quantity: line.qty })) } });
    setBusy(false);
  ...
  async function advanceRepair(repair: Repair) {
    ...
    if (next === "DELIVERED" && repair.paid < repair.estimated_cost && !window.confirm(`Qoldiq ${format(repair.estimated_cost - repair.paid)} to'lanmagan. Baribir topshirasizmi?`)) return;
    if (busy) return; setBusy(true);
  ...
  async function addTeamUser(event: FormEvent<HTMLFormElement>) {
    ...
    if (busy) return; setBusy(true);
    const res = await api<TeamUser>("/users", { method: "POST", body: { username: data.get("username"), password: data.get("password"), role: data.get("role"), full_name: data.get("full_name") || null, phone: data.get("phone") || null } });
  ...
  async function advanceOrder(id: number) {
    ...
    if (order && order.status === "Tayyor" && order.paid < order.total && !window.confirm(`Qoldiq ${format(order.total - order.paid)} to'lanmagan. Baribir topshirasizmi? (Qarz mijoz hisobida qoladi)`)) return;
    if (busy) return; setBusy(true);
  ...
  async function cancelOrder(id: number) {
    ...
    let refund = false;
    if (order && order.paid > 0) refund = window.confirm(`To'langan avans ${format(order.paid)} mijozga qaytarilsinmi?\n\nOK — avans qaytariladi (kassadan chiqim yoziladi)\nCancel — avans do'konda qoladi`);
    if (busy) return; setBusy(true);
    const res = await api<{ refunded: number }>(`/orders/${id}/cancel`, { method: "POST", body: { refund_advance: refund } });
    setBusy(false);
    if (!res.ok) return say(res.error, "error");
    await loadFromApi(); say(res.data.refunded ? `Buyurtma bekor qilindi — avans ${format(res.data.refunded)} mijozga qaytarildi` : "Buyurtma bekor qilindi va rezerv omborga qaytdi");
  }
  ...
  async function changeCashShift(event: FormEvent<HTMLFormElement>) {
    ...
    if (busy) return; setBusy(true);
    const res = await api<CashShift>(isOpening ? "/cash-shifts/open" : `/cash-shifts/${cashShift?.id}/close`, { method: "POST", body });
    setBusy(false);
    if (!res.ok) return say(res.error, "error");
    setModal(null); await loadFromApi();
    if (isOpening) return say("Kassa smenasi ochildi");
    const diff = res.data.difference ?? 0;
    say(`Smena yopildi. Kutilgan: ${format(res.data.expected_amount ?? 0)} · Haqiqiy: ${format(res.data.actual_amount ?? 0)} · ${diff === 0 ? "Farq yo'q ✓" : diff < 0 ? `KAMOMAD: ${format(-diff)}` : `Ortiqcha: ${format(diff)}`}`, diff === 0 ? "success" : "error");
  }
  async function returnSale(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(event.currentTarget);
    if (busy) return; setBusy(true);
    const res = await api<{ cash_refund: number; debt_reduction: number }>(`/sales/${selectedReturn.saleId}/return`, { method: "POST", body: { product_id: selectedReturn.product.id, quantity: Number(data.get("quantity")), disposition: data.get("disposition"), reason: data.get("reason") } });
    setBusy(false);
    if (!res.ok) return say(res.error, "error");
    const parts = [res.data.cash_refund ? `naqd qaytarildi: ${format(res.data.cash_refund)}` : "", res.data.debt_reduction ? `qarzdan yopildi: ${format(res.data.debt_reduction)}` : ""].filter(Boolean).join(" · ");
    setModal(null); await loadFromApi(); say(`Qaytarish saqlandi${parts ? ` — ${parts}` : ""}`);
  }
  async function addExpense(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(false);
    if (!res.ok) return say(res.error, "error");
    setModal(null); await loadFromApi(); say("Xarajat saqlandi — Kassa bo'limida ko'rinadi");
  }
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const body: Record<string, unknown> = { full_name: String(data.get("full_name") || ""), phone: String(data.get("phone") || ""), telegram_username: String(data.get("telegram_username") || "") };
    const newPass = String(data.get("new_password") || "");
    if (newPass) { body.new_password = newPass; body.current_password = String(data.get("current_password") || ""); }
    const file = data.get("photo") as File | null;
    if (file && file.size) {
      try { body.photo = await resizeImage(file); } catch { return say("Rasmni o'qib bo'lmadi — boshqa rasm tanlang", "error"); }
    }
    if (busy) return; setBusy(true);
    const res = await api<SessionUser>("/profile", { method: "PUT", body });
    setBusy(false);
    if (!res.ok) return say(res.error, "error");
    setUser(res.data); localStorage.setItem("optika-user", JSON.stringify(res.data));
    setModal(null); say("Profil saqlandi");
  }
  ...
  const modalTitles: Record<string, string> = {
    ...
    expense: "Xarajat kiritish", purchase: "Supplierdan kirim", user: "Yangi xodim", branch: "Yangi filial", profile: "Mening profilim",
  ...
  return <main className="app-shell">
    ...
    <section className="main">
      ...
      <div className="page">
        ...
        {section === "Bosh sahifa" && <Dashboard revenue={todayRevenue} debt={debt} customers={customers.length} debtors={customers.filter(c => c.debt > 0).length} orders={orders} lowStock={lowStock} go={goSection} setModal={setModal} statusTone={statusTone} userName={user.full_name || user.username}/>}
        ...
        {section === "Sotuv" && <POS products={products} cart={cart} total={cartTotal} query={query} add={addToCart} qty={updateQty} checkout={checkout} busy={busy} payMethod={payMethod} setPayMethod={setPayMethod} shiftOpen={!!cashShift} setModal={setModal}/>} 
        {section === "Sotuvlar" && <SalesHistory sales={sales} openReturn={(saleId, product, quantity, returned) => { setSelectedReturn({ saleId, product, quantity, returned }); setModal("return"); }}/>} 
        ...
        {section === "Kassa" && <Cash revenue={todayRevenue} debt={debt} shift={cashShift} expenses={expensesList} branchName={user.branch?.name} open={() => setModal("cash-open")} close={() => setModal("cash-close")} expense={() => setModal("expense")}/>} 
        {section === "Supplierlar" && <><div className="heading compact"><div><p>SUPPLIERLAR</p><h1>Yetkazib beruvchilar</h1><span>Kirim, to'lov va qarzdorlikni boshqaring.</span></div><div className="heading-actions"><button onClick={() => setModal("purchase")}>+ Kirim</button><button className="primary" onClick={() => setModal("supplier")}>+ Yangi supplier</button></div></div><section className="customer-grid">{suppliers.map(supplier => <article className="customer" key={supplier.id}><div className="customer-avatar">{supplier.name.slice(0,2).toUpperCase()}</div><div className="customer-info"><b>{supplier.name}</b><span>{supplier.phone || "Telefon kiritilmagan"}</span><small>{supplier.balance ? `Qarz: ${format(supplier.balance)}` : "Qarz yo'q"}</small></div>{supplier.balance > 0 && <button className="debt-action" onClick={() => { setSelectedSupplier(supplier); setModal("supplier-payment"); }}>To'lash</button>}</article>)}</section><section className="card money-card"><h2>So'nggi kirimlar</h2>{purchases.length ? purchases.slice(0, 10).map(p => <div key={p.id}><span>#{p.id} · {p.supplier.name}{p.invoice_no ? ` · ${p.invoice_no}` : ""} · {(parseApiDate(p.created_at) as Date).toLocaleDateString("uz-UZ")}</span><b>{format(p.total)}{p.balance > 0 ? ` (qarz: ${format(p.balance)})` : ""}</b></div>) : <div className="empty-line">Kirimlar hali yo'q</div>}</section></>} 
        {section === "Xodimlar" && <><div className="heading compact"><div><p>XO'JALIK / XODIMLAR</p><h1>Foydalanuvchilar va rollar</h1><span>Owner, manager va sotuvchi huquqlarini boshqaring.</span></div>{user.role === "OWNER" && <button className="primary" onClick={() => setModal("user")}>+ Xodim qo'shish</button>}</div><section className="customer-grid">{team.map(member => <article className="customer" key={member.id}>{member.photo ? <img className="customer-avatar avatar-photo" src={member.photo} alt=""/> : <div className="customer-avatar">{(member.full_name || member.username).slice(0,2).toUpperCase()}</div>}<div className="customer-info"><b>{member.full_name || member.username}</b><span>{member.role}{member.phone ? ` · ${member.phone}` : ""}</span><small>{member.username}{member.telegram_username ? ` · @${member.telegram_username}` : ""} · {member.active ? "Faol" : "Nofaol"}</small></div><span className={`badge ${member.active ? "ready" : "neutral"}`}>{member.active ? "Faol" : "Nofaol"}</span></article>)}</section>{!team.length && <div className="empty"><Users/><b>Bu bo'lim faqat Owner uchun</b></div>}</>} 
    ...
    {modal && <Modal title={modalTitle} close={() => setModal(null)}>
      ...
      {modal === "prescription" && <><p className="form-note">Mijoz: <b>{selectedCustomer?.name}</b> · {selectedCustomer?.phone}</p>{prescriptions.length > 0 && <div className="rx-history"><b>✓ Saqlangan retseptlar: {prescriptions.length} ta — oxirgisi formada turibdi, o'zgartirib saqlang</b>{prescriptions.slice(0, 3).map(p => <span key={p.id}>{(parseApiDate(p.created_at) as Date).toLocaleDateString("uz-UZ")} — OD: {p.od_sph || "—"} / {p.od_cyl || "—"} · OS: {p.os_sph || "—"} / {p.os_cyl || "—"}{p.pd != null ? ` · PD ${p.pd}` : ""}</span>)}{prescriptions.length > 3 && <span>… yana {prescriptions.length - 3} ta</span>}</div>}<form key={prescriptions[0]?.id ?? "new"} className="form prescription-form" onSubmit={addPrescription}><div className="eye-label">O'ng ko'z (OD)</div><div className="eye-grid"><label>SPH<input name="od_sph" placeholder="-2.75" defaultValue={prescriptions[0]?.od_sph ?? ""}/></label><label>CYL<input name="od_cyl" placeholder="-0.50" defaultValue={prescriptions[0]?.od_cyl ?? ""}/></label><label>AXIS<input name="od_axis" type="number" min="0" max="180" placeholder="90" defaultValue={prescriptions[0]?.od_axis ?? ""}/></label></div><div className="eye-label">Chap ko'z (OS)</div><div className="eye-grid"><label>SPH<input name="os_sph" placeholder="-1.00" defaultValue={prescriptions[0]?.os_sph ?? ""}/></label><label>CYL<input name="os_cyl" placeholder="-0.25" defaultValue={prescriptions[0]?.os_cyl ?? ""}/></label><label>AXIS<input name="os_axis" type="number" min="0" max="180" placeholder="90" defaultValue={prescriptions[0]?.os_axis ?? ""}/></label></div><label>PD (mm)<input name="pd" type="number" min="40" max="85" step="0.5" placeholder="62" defaultValue={prescriptions[0]?.pd ?? ""}/></label><label>Eslatma sanasi (keyingi tekshiruv)<input name="reminder_at" type="date" defaultValue={prescriptions[0]?.reminder_at?.slice(0, 10) ?? ""}/></label><label>Izoh<input name="note" placeholder="Qo'shimcha qayd" defaultValue={prescriptions[0]?.note ?? ""}/></label><button className="primary">{prescriptions.length ? "Retseptni yangilash" : "Retseptni saqlash"}</button></form></>}
      ...
      {modal === "user" && <form className="form" onSubmit={addTeamUser}><label>Login<input name="username" required placeholder="masalan, malika"/></label><label>Ism familiya<input name="full_name" placeholder="Masalan, Malika Karimova"/></label><label>Telefon<input name="phone" inputMode="tel" placeholder="90 123 45 67"/></label><label>Vaqtinchalik parol<input name="password" required type="password" minLength={6}/></label><label>Roli<select name="role"><option value="SELLER">Sotuvchi</option><option value="OPTOMETRIST">Optometrist</option><option value="LAB_QC">Laboratoriya / QC</option><option value="MANAGER">Manager</option><option value="OWNER">Owner</option></select></label><label>Filial<select name="branch_id" defaultValue=""><option value="">Filialsiz (butun tarmoq)</option>{branches.filter(branch => branch.active).map(branch => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><button className="primary">Xodimni yaratish</button></form>}
      {modal === "profile" && <div className="profile-body"><div className="profile-head">{user.photo ? <img className="profile-photo" src={user.photo} alt=""/> : <span className="profile-photo profile-initials">{user.username.slice(0,2).toUpperCase()}</span>}<div><b>{user.full_name || user.username}</b><span>{user.role}{user.branch ? ` · ${user.branch.name}` : ""}</span>{user.phone && <span>{user.phone}</span>}{user.telegram_username && <span>@{user.telegram_username}</span>}</div></div><form className="form" onSubmit={saveProfile}><label>Rasm (profil uchun)<input name="photo" type="file" accept="image/*"/></label><label>Ism familiya<input name="full_name" defaultValue={user.full_name ?? ""} placeholder="Masalan, Zamira Karimova"/></label><label>Telefon<input name="phone" inputMode="tel" defaultValue={user.phone ?? ""} placeholder="90 123 45 67"/></label><label>Telegram username<input name="telegram_username" defaultValue={user.telegram_username ?? ""} placeholder="@username"/></label><div className="form-split"><label>Joriy parol<input name="current_password" type="password" placeholder="O'zgartirish uchun"/></label><label>Yangi parol<input name="new_password" type="password" minLength={6} placeholder="Kamida 6 belgi"/></label></div><button className="primary" disabled={busy}>Profilni saqlash</button></form><button type="button" className="logout-btn" onClick={() => { setModal(null); signOut(); }}>Tizimdan chiqish</button></div>}
...
function POS({products,cart,total,query,add,qty,checkout,busy,payMethod,setPayMethod,shiftOpen,setModal}:{products:Product[];cart:CartLine[];total:number;query:string;add:(p:Product)=>void;qty:(id:number,c:number)=>void;checkout:()=>void;busy:boolean;payMethod:string;setPayMethod:(v:string)=>void;shiftOpen:boolean;setModal:(x:"customer")=>void}) { const visible = products.filter(p => `${p.name} ${p.brand} ${p.type}`.toLowerCase().includes(query.toLowerCase())); return <><div className="heading compact"><div><p>POS / SOTUV</p><h1>Yangi sotuv</h1><span>Mahsulotni savatchaga qo'shing va to'lovni yakunlang.</span></div><button onClick={() => setModal("customer")}>+ Yangi mijoz</button></div><div className="pos"><section className="catalog"><div className="catalog-title"><h2>Mahsulotlar</h2><span>{visible.length} ta natija</span></div><div className="product-grid">{visible.map(p => <button className="product" key={p.id} disabled={!p.stock} onClick={() => add(p)}><div className="product-image">{p.type === "Rama" ? "◯" : "◈"}</div><small>{p.brand} · {p.type}</small><b>{p.name}</b><strong>{format(p.price)}</strong><span className={p.stock <= p.min ? "stock low" : "stock"}>{p.stock} dona qoldi</span></button>)}</div></section><aside className="cart"><div className="cart-head"><div><h2>Savatcha</h2><span>{cart.length ? `${cart.length} xil mahsulot` : "Hali bo'sh"}</span></div><ShoppingBag size={19}/></div>{cart.length ? <><div className="cart-lines">{cart.map(line => <div className="cart-line" key={line.id}><div><b>{line.name}</b><span>{format(line.price)}</span></div><div className="qty"><button onClick={() => qty(line.id, -1)}><Minus size={14}/></button><b>{line.qty}</b><button onClick={() => qty(line.id, 1)}><Plus size={14}/></button></div><strong>{format(line.price * line.qty)}</strong></div>)}</div><div className="total"><span>Jami</span><strong>{format(total)}</strong></div>{!shiftOpen && <div className="shift-warning">Smena ochilmagan — bu sotuv kassa hisobiga tushmaydi. Kassa bo'limida smenani oching.</div>}<div className="pay-method">{[["CASH", "Naqd"], ["CARD", "Karta"], ["TRANSFER", "O'tkazma"]].map(([value, label]) => <button key={value} type="button" className={payMethod === value ? "active" : ""} onClick={() => setPayMethod(value)}>{label}</button>)}</div><button className="checkout" onClick={checkout} disabled={busy}><CreditCard size={18}/> To'lovni yakunlash</button></> : <div className="empty"><ShoppingBag/><b>Savatcha bo'sh</b><span>Chap tomondan mahsulot tanlang</span></div>}</aside></div></> }
function SalesHistory({sales,openReturn}:{sales:SaleRecord[];openReturn:(saleId:number,product:Product,quantity:number,returned:number)=>void}) { return <><div className="heading compact"><div><p>SOTUVLAR TARIXI</p><h1>Yakunlangan sotuvlar</h1><span>Mahsulot bo'yicha qaytarish shu yerdan amalga oshiriladi.</span></div></div><section className="card sale-history">{sales.length ? sales.map(sale => <article className="sale-history-row" key={sale.id}><div><b>Chek #{sale.id}</b><span>{sale.customer?.name || "Mehmon mijoz"} · {format(sale.total)}</span></div><div className="sale-items">{sale.items.map(item => <button key={item.product.id} disabled={item.returned >= item.quantity} onClick={() => openReturn(sale.id, item.product, item.quantity, item.returned)}><span>{item.product.name} × {item.quantity}{item.returned > 0 ? ` · qaytarilgan: ${item.returned}` : ""}</span><b>{item.returned >= item.quantity ? "Qaytarilgan" : "Qaytarish"}</b></button>)}</div></article>) : <div className="empty"><CreditCard/><b>Sotuvlar topilmadi</b></div>}</section></> }
...
function Cash({revenue,debt,shift,expenses,branchName,open,close,expense}:{revenue:number;debt:number;shift:CashShift | null;expenses:ExpenseRec[];branchName?:string;open:()=>void;close:()=>void;expense:()=>void}) {
  const MOVE_LABELS: Record<string, string> = { SALE_PAYMENT: "Sotuv", ORDER_ADVANCE: "Buyurtma avansi", REPAIR_PAYMENT: "Servis to'lovi", CUSTOMER_DEBT_PAYMENT: "Qarz to'lovi", REFUND: "Qaytarish", ORDER_REFUND: "Buyurtma bekori — avans qaytarildi", SUPPLIER_PAYMENT: "Supplierga to'lov", EXPENSE: "Xarajat" };
  const movements = shift?.movements || [];
  return <><div className="heading compact"><div><p>KASSA</p><h1>Bugungi kassa</h1><span>{shift ? `Smena #${shift.id} ochiq` : "Smena hozir yopiq"}{branchName ? ` · ${branchName}` : ""}</span></div><div className="heading-actions"><button onClick={expense}>+ Xarajat</button>{shift ? <button className="primary" onClick={close}>Smenani yopish</button> : <button className="primary" onClick={open}>Smenani ochish</button>}</div></div>{!shift && <div className="shift-warning">Smena ochilmagan — sotuv, to'lov va xarajatlar kassa hisobiga tushmaydi. Kun boshida smenani oching.</div>}<div className="cash-grid"><Metric icon={<CircleDollarSign/>} label="Kassada bo'lishi kerak" value={shift ? format(shift.expected ?? shift.opening_amount) : "—"} note={shift ? `Boshlang'ich: ${format(shift.opening_amount)}` : "Smena ochilmagan"}/><Metric icon={<ShoppingBag/>} label="Bugungi tushum" value={format(revenue)} note="Sotuv va buyurtma avansi"/><Metric icon={<CreditCard/>} label="Kutilayotgan qarz" value={format(debt)} note="Mijozlardan olinadi"/></div><section className="card money-card"><h2>{shift ? `Smena harakatlari (${movements.length} ta)` : "Smena harakatlari"}</h2>{movements.length ? movements.map(m => <div key={m.id}><span>{MOVE_LABELS[m.movement_type] || m.movement_type}{m.note ? ` · ${m.note}` : ""}</span><b className={m.amount < 0 ? "neg" : ""}>{m.amount < 0 ? "−" : "+"}{format(Math.abs(m.amount))}</b></div>) : <div className="empty-line">{shift ? "Hozircha pul harakati yo'q" : "Smena ochilganda harakatlar shu yerda ko'rinadi"}</div>}</section><section className="card money-card"><h2>So'nggi xarajatlar</h2>{expenses.length ? expenses.slice(0, 8).map(e => <div key={e.id}><span>{e.category}{e.description ? ` · ${e.description}` : ""} · {(parseApiDate(e.created_at) as Date).toLocaleDateString("uz-UZ")}</span><b className="neg">−{format(e.amount)}</b></div>) : <div className="empty-line">Xarajat kiritilmagan</div>}</section></>;
}
